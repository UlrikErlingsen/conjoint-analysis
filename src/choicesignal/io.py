...
MAX_UPLOAD_MB = max(1, min(int(os.getenv("CHOICESIGNAL_MAX_UPLOAD_MB", "200")), 1000))
...
def load_data(source: str | Path | bytes | BinaryIO, name: str | None = None) -> LoadedData:
    ...
    if len(raw) > MAX_UPLOAD_BYTES:
        raise DataProblem(
            ...
            "Reduce it to the respondent, attribute, and rating columns you need."
        )
